...
# Paddle settings
PADDLE_WIDTH = SCREEN_WIDTH // 7  # Paddle width (default: 1/7th of the screen width)
...
ball_dx = 0  # Initially 0 to keep the ball stationary
ball_dy = 0  # Initially 0 to keep the ball stationary
...
def reset_paddle_and_ball():
    ...
    ball_dx = 0  # Reset ball speed to 0 to keep it stationary
    ball_dy = 0
    ball_speed_multiplier = BALL_SPEED_MULTIPLIER
...
def next_level():
    ...
    ball_speed_multiplier = BALL_SPEED_MULTIPLIER
    ball_dx = 0  # Reset ball speed to 0 to prepare for the next level
    ball_dy = 0
...
# Function to move the paddle using arrow keys
def move_paddle():
    global paddle_x
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_F8:
                exit_game()
            if event.key == pygame.K_SPACE and ball_dx == 0 and ball_dy == 0:
